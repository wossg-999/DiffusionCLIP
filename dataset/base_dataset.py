...
"""
import sys
sys.path.append('D:\gitku\AD Codes\AdaCLIP')
import json
import os
import random
import numpy as np
...
class DataSolver:
    ...
    def run(self):
        ...
        info_required = dict(train={}, test={})
        for cls in self.clsnames:
            for phase in info.keys():  # 遍历 train 和 test
                if cls in info[phase]:  # 检查该类是否在该阶段中存在
                    info_required[phase][cls] = info[phase][cls]
                else:
                    print(f"Warning: Class '{cls}' not found in {phase} phase.")
...
class BaseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if img_mask is None:
            img_mask = []
        # if img is None:
        #     print(f"Error: Image at {img_path} failed to load.")
